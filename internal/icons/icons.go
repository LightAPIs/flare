...
import (
	...
	FlareSI "github.com/soulteary/flare/internal/resources/simpleicons"
	FlareState "github.com/soulteary/flare/internal/state"
)

var MemFs *memfs.FS

const _ASSETS_BASE_DIR = "assets/icons"
const _ASSETS_WEB_URI = "/" + _ASSETS_BASE_DIR

// 缓存图标
var _CACHE_ICONS_ITEM_EXIST map[string]bool
var _CACHE_ICONS_ITEM_DATA map[string]string
...
func Init() {
	...
	_CACHE_ICONS_ITEM_EXIST = make(map[string]bool)
	_CACHE_ICONS_ITEM_DATA = make(map[string]string)
}
...
func GetIconByName(name string) string {
	...
	icon := FlareMDI.IconMap[strings.ToLower(name)]
	if icon == "" {
		icon = FlareSI.IconMap[strings.ToLower(name)]
		if icon == "" {
			return _EMPTY_ICON
		}
	}
	...
	if FlareState.AppFlags.EnableMinimumRequest {
		if !_CACHE_ICONS_ITEM_EXIST[name] {
			content = `<svg viewBox="0 0 24 24"><path d="` + icon + `" style="fill: var(--color-primary);"></path></svg>`
			_CACHE_ICONS_ITEM_DATA[name] = content
			_CACHE_ICONS_ITEM_EXIST[name] = true
		}
		return _CACHE_ICONS_ITEM_DATA[name]
	}

	svgFile := filepath.ToSlash(filepath.Join(_ASSETS_BASE_DIR, (FlareState.ThemeCurrent + "-" + name + ".svg")))
	if !_CACHE_ICONS_ITEM_EXIST[FlareState.ThemeCurrent+"-"+name] {
		...
		_CACHE_ICONS_ITEM_EXIST[FlareState.ThemeCurrent+"-"+name] = true
	}
