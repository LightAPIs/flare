...
func TestParse(t *testing.T) {
	...
	expectedFlags.LogLevel = defaults.LogLevel
...
func TestExcuteCLI_ShowHelp(t *testing.T) {
	...
	output := captureOutput(func() {
		_ = ExcuteCLI(cliFlags, options)
	})

	assert.Contains(t, output, "支持命令：", "应该打印出支持命令")
	assert.True(t, ExcuteCLI(cliFlags, options), "在 ShowHelp 为 true 时，应该返回 true")
}
...
func TestExcuteCLI_ShowVersion(t *testing.T) {
	...
	assert.True(t, ExcuteCLI(cliFlags, options), "在 ShowVersion 为 true 时，应该返回 true")
}
...
func TestExcuteCLI_NoFlags(t *testing.T) {
	...
	assert.False(t, ExcuteCLI(cliFlags, options), "当没有任何标志被设置时，应该返回 false")
}
